-- Function: invoiceOpenFTA(numeric, numeric)

-- DROP FUNCTION invoiceOpenFTA(numeric, numeric);

CREATE OR REPLACE FUNCTION invoiceOpenFTA(p_c_invoice_id numeric, p_c_invoicepayschedule_id numeric)
  RETURNS numeric AS
$BODY$
/*************************************************************************
...
BEGIN
	...
	LOOP
        ...
        -- Allocation
			+ currencyConvert(v_Temp * v_MultiplierAP,
				ar.C_Currency_ID, v_Currency_ID, ar.DateDoc, null, ar.AD_Client_ID, ar.AD_Org_ID);
      	RAISE NOTICE '   PaidAmt=% , Allocation= % * %', v_PaidAmt, v_Temp, v_MultiplierAP;
	END LOOP;
