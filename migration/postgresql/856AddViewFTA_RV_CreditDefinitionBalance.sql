...
SELECT cd.FTA_CreditDefinition_ID, cd.AD_Client_ID, ft.AD_Org_ID, cd.Created, cd.CreatedBy, cd.Updated, cd.UpdatedBy, cd.IsActive,
...
GROUP BY cd.FTA_CreditDefinition_ID, cd.AD_Client_ID, ft.AD_Org_ID, cd.Created, cd.CreatedBy, 
cd.Updated, cd.UpdatedBy, cd.IsActive,
cd.C_DocType_ID, cd.DateDoc, cd.DocumentNo, cd.Description, cd.DocStatus, cd.Amt, cd.Category_ID,
cd.M_PriceList_ID, cd.PlantingCycle_ID, 
cdl.FTA_CreditDefinitionLine_ID, cdl.Line, 
cdl.M_Product_Category_ID, cdl.M_Product_ID, cdl.C_Charge_ID, pc.Name, pr.Name, cr.Name,
cdl.Amt, fc.C_BPartner_ID, ft.AD_Table_ID, fc.FTA_FarmerCredit_ID, ct.Name,fc.ApprovedQty, cdlc.Name, cp.C_BP_Group_ID