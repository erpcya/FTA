	...
	implements ListSelectionListener
{
	...
	private CCheckBox showCategoryCalcOnly = new CCheckBox (Msg.translate(Env.getCtx(), "FTA_CategoryCalc_ID"));
	//
	...
	private void jbInit() throws Exception
	{
		...
		northPanel.add(showCategoryCalcOnly, BorderLayout.EAST);
		showCategoryCalcOnly.addActionListener(this);
		//	End Yamel Senih
...
//		new ColumnInfo(Msg.translate(Env.getCtx(), "M_AttributeSet_ID"), "st.Name", KeyNamePair.class, "st.M_AttributeSet_ID"), // @Trifon - Not sure if this need to be shown
	...
	private void refresh()
	{
		...
		//	Add Join
		if(showCategoryCalcOnly.isSelected()){
			String sql2 = sql;
			pos = sql2.indexOf(" LEFT OUTER JOIN");
			sql = sql2.substring(0, pos) 
					+ " INNER JOIN FTA_CategoryCalc cc ON (cc.M_AttributeSetInstance_ID = asi.M_AttributeSetInstance_ID " 
					+ " 										AND cc.M_Product_ID = " + m_M_Product_ID + ")";
				sql += sql2.substring(pos);
		}
		//	End Yamel Senih
		//
		log.finest(sql);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			refresh();
		} else if(e.getSource() == showCategoryCalcOnly) {
			refresh();
		}
	}
