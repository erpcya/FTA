 ...
 *
 */
public class MFTAFarmerLiquidation extends X_FTA_FarmerLiquidation implements DocAction, DocOptions {
	...
	/** Lines					*/
	private MFTAFarmerLiquidationLine[]		m_lines = null;
	/**
	 * 	Get Document Info
	...
	public String completeIt()
	{
		...
		//	Very Lines
		MFTAFarmerLiquidationLine[] lines = getLines(false);
		if (lines.length == 0){
	...
	private String validReference(){
		int m_Reference_ID = DB.getSQLValue(get_TrxName(), "SELECT MAX(a.FTA_Allocation_ID) " +
				...
				"AND a.FTA_FarmerLiquidation_ID = ?", getFTA_FarmerLiquidation_ID());
		if(m_Reference_ID != 0)
	 ...
	 *	@return lines
	 */
	public MFTAFarmerLiquidationLine[] getLines (boolean requery)
	{
		...
		List<MFTAFarmerLiquidationLine> list = new Query(getCtx(), 
				MFTAFarmerLiquidationLine.Table_Name, "FTA_FarmerLiquidation_ID=?", get_TrxName())
		.setParameters(getFTA_FarmerLiquidation_ID())
		.list();

		m_lines = new MFTAFarmerLiquidationLine[list.size ()];
    ...
    public void setProcessed (boolean processed)
    {
        ...
        if (get_ID() <= 0)
            return;
        int noLine = DB.executeUpdateEx("UPDATE FTA_FarmerLiquidationLine " +
        		"SET Processed=? " +
        		"WHERE FTA_FarmerLiquidation_ID=?",
