...
public class CalloutQualityAnalysis extends CalloutEngine
{
	 ...
	 * 
	 * @author <a href="mailto:yamelsenih@gmail.com">Yamel Senih</a> 05/09/2013,
	 *         11:43:33
	 * @contributor <a href="mailto:dixon.22ma@gmail.com">Dixon Martinez</a>
	 *              12/12/2013, 16:00:01 <li>Add Validation Operation Type</li>
	 ...
	 * @return String
	 */
	public String entryTicket(Properties ctx, int WindowNo, GridTab mTab,
			GridField mField, Object value)
	{
		...
		if (m_FTA_EntryTicket_ID == null
				|| m_FTA_EntryTicket_ID.intValue() == 0)
			return "";
		...
		if (!mField.get_ValueAsString("OperationType").equals(
				X_FTA_QualityAnalysis.OPERATIONTYPE_RawMaterialReceipt))
			return "";

		// get Mobilization Guide
		String sql = new String(
				"SELECT fg.Category_ID "
						+ "FROM FTA_Farming fg "
		...
		//
		if (m_AnalysisType != null && m_AnalysisType.equals("CA"))
		{
			//
			sql = "SELECT qa.FTA_QualityAnalysis_ID "
					...
					+ "WHERE et.FTA_EntryTicket_ID = ?";
			//
			int m_Orig_QualityAnalysis_ID = DB.getSQLValue(null, sql,
					m_FTA_EntryTicket_ID);
			...
			sql = "SELECT FTA_RecordWeight_ID " + "FROM FTA_RecordWeight "
					+ "WHERE FTA_EntryTicket_ID = ? "
					+ "AND DocStatus IN('DR', 'IP', 'IN')";
			int m_FTA_RecordWeight_ID = DB.getSQLValue(null, sql,
					m_FTA_EntryTicket_ID);
	 ...
	 * @return String
	 */
	public String recordWeight(Properties ctx, int WindowNo, GridTab mTab,
			GridField mField, Object value)
	{
		...
		MFTARecordWeight m_FTA_RecordWeight = new MFTARecordWeight(
				Env.getCtx(), p_FTA_RecordWeight, null);
		...
		} else if (mField.get_ValueAsString("OperationType").equals(
				X_FTA_QualityAnalysis.OPERATIONTYPE_DeliveryBulkMaterial))
		{// Set Product of Load Order
			// Create Load Order Object
			MFTALoadOrder m_FTA_LoadOrder = new MFTALoadOrder(Env.getCtx(),
					m_FTA_RecordWeight.getFTA_LoadOrder_ID(), null);
